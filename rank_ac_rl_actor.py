...
from model_ops.bert import transformer_model_v2
...
# based on YuexiaACRLAfcActor, item_score_seq -> actor_item_score_seq, critic_item_score_seq -> item_score_seq

class YuexiaACRLTransformerActor(YuexiaACRLTransformerBase):
    def __init__(self,
                 model_config,
                 training_config,
                 mc,
                 ...
                 context,
                 name="CTR"):
        super(YuexiaACRLTransformerActor, self).__init__(
            model_config,
            training_config,
            mc,
            fg,
            context,
            name)
        logging.info("[YuexiaACRLTransformerActor|init] model: {}, init start.".format(self.name))
        ...
        self.context_column_blocks = ['user_columns', 'account_columns', 'account_cross_columns', 'context_columns']
        self.column_blocks = ['critic_account_columns', 'account_id_columns', 'top2_ta_query'] + self.main_column_blocks + self.bias_column_blocks
        ...
        self.seq_column_blocks = ['actor_item_score_seq', 'item_score_seq', 'actor_item_query_seq', 'item_query_seq'] + self.actor_seq_column_blocks + self.ta_seq_column_blocks
        self.column_blocks += [seq_name + '_length' for seq_name in self.seq_column_blocks]

        self.item_column_blocks = ['actor_item_score_seq']
        self.item_query_blocks = ['actor_item_query_seq']
    ...
    def build(self, features, feature_columns, labels):
        logging.info("[YuexiaACRLTransformerActor|build] model: {}, build start.".format(self.name))

        super(YuexiaACRLTransformerActor, self).build(features, feature_columns, labels)
    ...
    def build_model(self):
        logging.info("[YuexiaACRLTransformerActor|build_model] model: {}, build_model start.".format(self.name))
    ...
    def model_graph(self):
        logging.info("[YuexiaACRLTransformerActor|model_graph] model: {}, model_graph start.".format(self.name))
        ...
        logging.info("[YuexiaACRLTransformerActor|model_graph] model: {}, model_graph end.".format(self.name))

    def loss_op(self):
        logging.info("[YuexiaACRLTransformerActor|loss_op] model: {}, loss_op start.".format(self.name))
        with tf.name_scope("{}_Loss_Op".format(self.name)):
            self.loss = self.rl_loss + self.reg_loss

    def encoder(self):
        logging.info("[YuexiaACRLTransformerActor|encoder] model: {}, encoder start.".format(self.name))
        ...
        if not self.seq_block_layer_dict.has_key(block_name):
            ...
            return
        logging.info("[YuexiaACRLTransformerActor|encoder] model: {}, encoder block_name: {}".format(self.name, block_name))
        ...
        with arg_scope(base_ops.model_arg_scope(weight_decay=self.training_config.dnn_l2_reg)):
            with tf.variable_scope(name_or_scope="encoder",
                ...
                logging.info("[YuexiaACRLTransformerActor|encoder] model: {}, item_score_seq_feat: {}".format(self.name,  self.item_score_seq_feat))
                ...
                for seq_name in self.actor_seq_column_blocks:
                    ...
                    if not self.block_layer_dict.has_key(column_name):
                        logging.warn('[YuexiaACRLTransformerActor|encoder] model: {}, block_layer_dict does not has block: {}'.format(self.name, column_name))
                        continue
                    logging.info("[YuexiaACRLTransformerActor|encoder] model: {}, item_embeddings add feat {} : {}".format(self.name, column_name, self.block_layer_dict[column_name]))
                ...
                logging.info("[YuexiaACRLTransformerActor|encoder] model: {}, sequence_mask: {}".format(self.name, self.sequence_mask))
                ...
                logging.info("[YuexiaACRLTransformerActor|encoder] model: {}, item_embeddings: {}".format(self.name, self.item_embeddings))

        logging.info("[YuexiaACRLTransformerActor|encoder] model: {}, encoder end.".format(self.name))
    ...
    def decoder(self):
        logging.info("[YuexiaACRLTransformerActor|decoder] model: {}, decoder start.".format(self.name))
        ...
        with arg_scope(base_ops.model_arg_scope(weight_decay=self.training_config.dnn_l2_reg)):
            with tf.variable_scope(name_or_scope="decoder",
                ...
                logging.info("[YuexiaACRLTransformerActor|decoder] model: {}, item_k_embedding: {}".format(self.name, self.item_k_embedding))
                logging.info("[YuexiaACRLTransformerActor|decoder] model: {}, item_v_embedding: {}".format(self.name, self.item_v_embedding))
                logging.info("[YuexiaACRLTransformerActor|decoder] model: {}, item_k_embedding: {}".format(self.name, self.item_l_embedding))
                ...
                logging.info("[YuexiaACRLTransformerActor|decoder] model: {}, graph_embedding: {}".format(self.name, self.graph_embedding))

                context_feat_list = []
                for block_name in self.context_column_blocks:
                    if not self.block_layer_dict.has_key(block_name):
                        logging.warn('[YuexiaACRLTransformerActor|decoder] model: {}, block_layer_dict does not has block: {}'.format(self.name, block_name))
                        continue
                ...
                logging.info("[YuexiaACRLTransformerActor|decoder] model: {}, context_feat: {}".format(self.name, self.context_feat))
                logging.info("[YuexiaACRLTransformerActor|decoder] model: {}, concat(context_embedding, context_feat): {}".format(self.name, self.context_embedding))
                ...
                logging.info("[YuexiaACRLTransformerActor|decoder] model: {}, context_embedding: {}".format(self.name, self.context_embedding))

                for seq_name in self.actor_seq_column_blocks:
                    column_name = seq_name + '_pooling'
                    if column_name not in self.block_layer_dict:
                        logging.warn('[YuexiaACRLTransformerActor|decoder] model: {}, block_layer_dict does not has block: {}'.format(self.name, column_name))
                        continue
                    add_feat = tf.expand_dims(self.block_layer_dict[column_name], axis=1)
                    logging.info("[YuexiaACRLTransformerActor|decoder] model:{}, context_embedding add feat {} : {}".format(self.name, column_name, add_feat))
                    self.context_embedding = tf.add(self.context_embedding, add_feat)

        logging.info('[YuexiaACRLTransformerActor|decoder] decoder end.')

    def rollout(self, strategy):
        logging.info('[YuexiaACRLTransformerActor|rollout] rollout start, strategy: {}'.format(strategy))
        ...
        logging.info('[YuexiaACRLTransformerActor|rollout] rollout end, strategy: {}'.format(strategy))
        return log_p, pi
    ...
    def get_log_p(self, q, k, v, l, mask):
        logging.info('[YuexiaACRLTransformerActor|get_log_p] get_log_p start.')
    ...
    def metrics_op(self):
        with tf.name_scope("{}_Metrics".format(self.name)):
            ...
            logging.info("[YuexiaACRLTransformerActor|get_log_p] metrics: {}".format(self.metrics))
    ...
    def summary_op(self):
        super(YuexiaACRLTransformerActor, self).summary_op()
        ...
        with tf.name_scope('RL_Sequence_Embedding_Summary'):
            for block_name, layer in self.seq_block_layer_dict.items():
                ...
                logging.info('[YuexiaACRLTransformerActor|summary_op] model: {}, seq embedding block_name: {} and columns: {}'.format(self.name, block_name, self.mc.get_seq_column_names_by_block_name(block_name)))
    ...
    def evaluate(self, pi):
        logging.info('[YuexiaACRLTransformerActor|evaluate] model: {}, evaluate start.'.format(self.name))

        selected_item_emb = self.critic_embedding_layer('item_score_seq', pi)
        self.update_column2emb_dict['critic_item_columns'] = selected_item_emb

        account_id_emb = self.block_layer_dict['account_id_columns']
        selected_ta_item_emb = self.critic_embedding_layer('item_query_seq', pi)
        selected_ta_query_emb = tf.concat([account_id_emb, selected_ta_item_emb], axis=-1)
        self.update_column2emb_dict['ta_query'] = selected_ta_query_emb
        logging.info('[YuexiaACRLTransformerActor|evaluate] model: {}, selected_ta_query_emb shape: {}'.format(self.name, selected_ta_query_emb.get_shape().as_list()))
        ...
        for key, weight in self.config.rl_weights.items():
            logging.info("[YuexiaACRLTransformerActor|evaluate] model: {}, key: {}, weight: {}".format(self.name, key, weight))
    ...
    def evaluate_orig(self):
        logging.info('[YuexiaACRLTransformerActor|evaluate_orig] model: {}, evaluate_orig start.'.format(self.name))
        ...
        logging.info('[YuexiaACRLTransformerActor|evaluate_orig] model: {}, orig_selected_item_emb shape: {}'.format(
            self.name, orig_selected_item_emb.get_shape().as_list()))
        logging.info('[YuexiaACRLTransformerActor|evaluate_orig] model: {}, orig_selected_ta_query_emb shape: {}'.format(
        ...
        rewards = 0
        for key, weight in self.config.rl_weights.items():
            logging.info("[YuexiaACRLTransformerActor|evaluate_orig] model: {}, key: {}, weight: {}".format(self.name, key, weight))
            if key in self.config.esmm_dict:
    ...
    def critic_embedding_layer(self, column_name, pi, dim=None):
        logging.info('[YuexiaACRLTransformerActor|critic_embedding_layer] model: {}, critic_embedding_layer start.'.format(self.name))
        with tf.variable_scope(name_or_scope="input_from_feature_columns",
                               partitioner=base_ops.partitioner(self.config.ps_num,
                                                                self.training_config.embedding_partition_size),
                               reuse=tf.AUTO_REUSE) as scope:
            seq_feature = self.seq_block_layer_dict[column_name]
    ...
    def actor_sequence_layer(self):
        logging.info("[YuexiaACRLTransformerActor|actor_sequence_layer] model: {}, actor_sequence_layer start.".format(self.name))
        with tf.variable_scope(name_or_scope="{}_rl_model".format(self.name),
                               partitioner=base_ops.partitioner(self.config.ps_num,
                                                                self.training_config.dnn_partition_size),
                               reuse=tf.AUTO_REUSE):
            self.query_name = self.item_query_blocks[0]
        ...
        logging.info("[YuexiaACRLTransformerActor|actor_sequence_layer] model: {}, actor_sequence_layer end.".format(self.name))

    def actor_seq_pooling_layer(self):
        # long seq: mean -> fc; others: fc -> mean
        logging.info("[YuexiaACRLTransformerActor|actor_seq_pooling_layer] model: {}, actor_seq_pooling_layer start.".format(self.name))
        ...
        for block_name in self.actor_seq_column_blocks:
            ...
            with arg_scope(base_ops.model_arg_scope(weight_decay=self.training_config.dnn_l2_reg)):
                with tf.variable_scope(name_or_scope=block_name + "_seq_pooling_layer",
                    ...
                    logging.info("[YT DEBUG | seq] {}_pooling_layer: {}".format(block_name, self.block_layer_dict[block_name + '_pooling']))

    def actor_seq_cross_atten_layer(self):
        logging.info("[YuexiaACRLTransformerActor|actor_seq_cross_atten_layer] model: {}, actor_seq_cross_atten_layer start.".format(self.name))
        for key_name in self.short_seq_columns:
            if not self.seq_block_layer_dict.has_key(key_name):
                logging.warn('[YuexiaACRLTransformerActor|actor_seq_cross_atten_layer] seq_block_layer_dict dict does not has block: {}'.format(self.name, key_name))
                continue
            with arg_scope(base_ops.model_arg_scope(weight_decay=self.training_config.dnn_l2_reg)):
                with tf.variable_scope(name_or_scope=key_name + "_cross_attention_layer",
                    ...
                    output = feedforward(
                        item_vec,
                        num_units=[self.training_config.att_num_units * 2, self.training_config.att_num_units],  # [64, 128]
                    ...
                    logging.info("[YT DEBUG | seq] {}_cross_attention_layer: {}".format(key_name, self.block_layer_dict[key_name + '_target_atten']))

    def actor_long_seq_cross_atten_layer(self):
        logging.info("[YuexiaACRLTransformerActor|actor_long_seq_cross_atten_layer] model: {}, actor_long_seq_cross_atten_layer start.".format(self.name))
        for key_name in self.long_seq_columns:
            if not self.seq_block_layer_dict.has_key(key_name):
                logging.warn('[YuexiaACRLTransformerActor|actor_long_seq_cross_atten_layer] model: {}, seq_block_layer_dict dict does not has block: {}'.format(self.name, key_name))
                continue
            with arg_scope(base_ops.model_arg_scope(weight_decay=self.training_config.dnn_l2_reg)):
                with tf.variable_scope(name_or_scope=key_name + "_cross_attention_layer",
                                       partitioner=base_ops.partitioner(self.config.ps_num,
                                                                        self.training_config.dnn_partition_size),
                                       reuse=tf.AUTO_REUSE) as scope:
                    logging.info("[YuexiaACRLTransformerActor|actor_long_seq_cross_atten_layer] model: {}, add {}_cross_attention_layer begin".format(self.name, key_name))
                    # key
                    key_max_len = self.fg.get_seq_len_by_sequence_name(key_name)
                    key_length = self.block_layer_dict['{}_length'.format(key_name)]
                    ...
                    poly_cov_long = (tf.ones_like(poly_cov_long) - tf.eye(self.training_config.ncodes)) * poly_cov_long  # shape=(code, code)
                    self.cov_loss_long = tf.reduce_sum(tf.abs(poly_cov_long))

                    # Attention Query Build
                    poly_matrix_long_tile = tf.tile(tf.expand_dims(poly_matrix_long, 0), [tf.shape(key_seq)[0], 1, 1])  # shape=(batch, code, fdim)
                    ...
                    atten_query_mask = tf.sequence_mask(tf.reshape(atten_query_length, [-1]), atten_query_max_len)  # [B, L_q]
                    ...
                    output = feedforward(
                        item_vec,
                        num_units=[self.training_config.att_num_units * 2, self.training_config.att_num_units],  # [256, 128]
                        activation_fn=utils.getActivationFunctionOp(self.config.activation_op),
                        scope="feed_forward",
                        reuse=tf.AUTO_REUSE,
                        variables_collections=[self.rl_long_seq_cross_att_collections_dnn_hidden_layer],
                    ...
                    self.block_layer_dict[key_name + '_target_atten'] = output
                    logging.info("[YuexiaACRLTransformerActor|actor_long_seq_cross_atten_layer] model: {}, {}_cross_attention_layer: {}".format(self.name, key_name, self.block_layer_dict[key_name + '_target_atten']))